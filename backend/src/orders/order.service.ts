...
export const createOrder = async (items: any[], userId: Schema.Types.ObjectId) => {
  ...
  try{
  const findProduct = await productDal.findById(items[0].productId)
  if(!findProduct) throw new AppError("No se enviaron productos para crear el pedido", 400);
  const sellerId: Schema.Types.ObjectId = findProduct.userId;

  //  Mapeamos y procesamos el stock de cada item
  ...
  await Promise.all(
    items.map(async (item) => {
      ...
      // Si el DAL devuelve null, es porque no había stock suficiente
      if (!updatedProduct) throw new AppError(`Stock insuficiente para el producto con ID: ${item.productId}`,400);
    })
  );
  //  Una vez restado el stock de todos, creamos la orden
...
export const updateStatus = async (id: string, status: string) => {
  ...
  if(!updatedOrder) throw new AppError("Pedido no encontrado", 404);

  // Enviar notificación al usuario sobre el cambio de estado
  await sendNotification({
      user: updatedOrder.sellerId.toString(),
      title: `Estado de pedido actualizado a ${status}`,
      message: `Tu pedido ha sido actualizado a estado ${status}.`,
      link: `/orders/detail/${id}`
    });
  
    await sendNotification({
      user: updatedOrder.buyerId.toString(),
      title: `Estado de pedido actualizado a ${status}`,
      message: `Tu pedido ha sido actualizado a estado ${status}.`,
      link: `/orders/detail/${id}`
    });
  ...
  return updatedOrder;
    ...
    throw new AppError(`Error inesperado al actualizar el estado del pedido: ${error}`, 500)
  }
...
export const getOrdersByBuyerId = async (userId: string) => {
  ...
  if(!orders) throw new AppError(`No se encontraron compras para este usuario ${userId}`, 404);
  return orders;
};
