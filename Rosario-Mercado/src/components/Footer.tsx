...
export const Footer: React.FC = () => {
  ...
  const activeBtnClasses = "flex flex-col items-center justify-center text-red-500 gap-1";
  ...
  return (
    <footer className="fixed bottom-0 left-0 w-full bg-white rounded-t-3xl shadow-[0_-4px_10px_rgba(0,0,0,0.05)] z-50 pb-[env(safe-area-inset-bottom)]">
      <div className="flex justify-around items-center h-[70px] px-2 relative">
        ...
        {/* --- Botón Inicio (Activo) --- */}
        <button className={navBtnClasses} onClick={() => navigate('/')}>
        ...
        {/* --- Botón Compras --- */}
        <button className={navBtnClasses} onClick={() => navigate('/orders/user')} disabled={!user}>
        ...
        {/* --- Botón Perfil --- */}
        <button className={navBtnClasses} onClick={() => navigate('/dashboard')} disabled={!user}>
          <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-6 h-6">
