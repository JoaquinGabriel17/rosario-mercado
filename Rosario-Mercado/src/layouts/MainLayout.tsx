...
const MainLayout = () => {
  ...
  const [notification, setNotification] = useState<{title: string, type: string ,message: string, link: string} | null>(null);
  ...
  return (
    <>
      <Navbar />
      {notification && <div onClick={() => navigate(notification.link)}> <Toaster position="top-right"/> </div>}
      <div className="mb-20">
      <Outlet />
      </div>
      <Footer />
